...
//     if (bill.user.toString() !== req.user.id.toString()) {
//       return res.status(401).json({ message: "Not authorized" });
//     }
...
//     if (user.monthlyBudget > 0 && totalPaid.length > 0) {
//       if (totalPaid[0].total > user.monthlyBudget) {
//         budgetWarning = `⚠ You crossed your ₹${user.monthlyBudget} monthly budget!`;
//       }
//     }

//     res.json({ bill, budgetWarning });
...
//     if (!bill.paid) {
//       return res.status(400).json({ message: "Only paid bills can be deleted" });
//     }
...
router.put("/:id", auth, async (req, res) => {
  try {
    ...
    // ✅ ONLY UPDATE STATUS (no recurring here now)
    bill.paid = req.body.paid;
    await bill.save();
